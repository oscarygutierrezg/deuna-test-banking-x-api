...
func TestCreateOrderSuccess(t *testing.T) {
	...
	businessDestination := entity.BusinessDestination{
	...
	municipality := entity.Municipality{
		...
		Region:  "Santiago",
		Name:    "Santiago",
		Kind:    entity.MunicipalityBase,
	}

	fee := entity.Fee{
		WeightRange: entity.WeightRange0610,
		Price:       2000,
	}
	...
	order := entity.Order{
		...
		PackageQty:      3,
		WeightRange:     entity.WeightRange0610,
		Origin:          entity.BusinessOrigin{},
		Destination:     entity.BusinessDestination{},
	}
	...
	assert.Equal(entity.OrderAvailable, resp.Data.(map[string]interface{})["state"])
}
...
func TestListOrderWeightRangesSuccess(t *testing.T) {
	...
	assert.NotNil(resp.Data)
	assert.IsType([]interface{}{}, resp.Data)
	assert.ElementsMatch(resp.Data, entity.WeightRanges)
}
...
func TestMassiveUploadOrderAndVerifyPricesSuccess(t *testing.T) {
	...
	var origins []entity.BusinessOrigin
	...
	for i, row := range xlsRows {
		...
		if len(row) >= 12 {
			origins = append(origins, entity.BusinessOrigin{
				Name:            row[12],
				Address:         "La Moneda 970",
				AddressOptional: "Dpto. 710",
				Country:         "",
...
func TestCreateOrderNextStateHappyWaySuccess(t *testing.T) {
	...
	workLoad, _ := entity.NewWorkLoad(entity.WorkLoad{})
	...
	workLoad.Orders = []entity.WorkLoadOrder{
		{
			OrderIndex: 0,
			OrderId:    resp.Data.(map[string]interface{})["_id"].(string),
			OrderState: entity.OrderAssigned,
	...
	nextStateRequest.Body = map[string]interface{}{
		"type":  entity.OrderTypeOrigin,
		"state": entity.OrderReqSuccess,
	}
	_, resp, nextStateErr := simulateJSONRequest(router, nextStateRequest, true)
	assert.Nil(nextStateErr)
	assert.Equal(entity.OrderPickedUp, resp.Data.(map[string]interface{})["newState"].(string))
	...
	assert.Equal(entity.OrderPickedUp, workLoad.Orders[0].OrderState)
	assert.Equal(entity.OrderAssigned, workLoad.Orders[1].OrderState)

	// OrderDelivered
	nextStateRequest.Body = map[string]interface{}{
		"type":  entity.OrderTypeDestination,
		"state": entity.OrderReqSuccess,
	}
	_, resp, nextStateErr = simulateJSONRequest(router, nextStateRequest, true)
	assert.Nil(nextStateErr)
	assert.Equal(entity.OrderDelivered, resp.Data.(map[string]interface{})["newState"].(string))

	oId, _ = primitive.ObjectIDFromHex(workLoad.Id)
	findWorkLoadErr = db.FindOneStruct("work-load", mongodb.Query{Selector: bson.M{"_id": oId}}, &workLoad)
	assert.Nil(findWorkLoadErr)
	assert.Equal(entity.OrderPickedUp, workLoad.Orders[0].OrderState)
	assert.Equal(entity.OrderDelivered, workLoad.Orders[1].OrderState)
}
